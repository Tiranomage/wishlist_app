...
export function render(items: any[]) {
	...
	if (items.length === 0) {
		container.innerHTML = '<div class="card"><p>No wishlists yet. Create your first wishlist!</p></div>';
		return;
	}
	...
	for (const w of items) {
		...
		el.className = 'card';
		el.innerHTML = `
			<div class="row" data-act="open" data-id="${w.id}">
				<h3 style="margin:0; flex:1;">${w.title}</h3>
				<span class="badge ${w.is_public ? 'public':'private'}">${w.is_public ? 'Public':'Private'}</span>
				<span class="badge">${w.gift_count||0} gifts</span>
			</div>
			<p>${w.description||'<em>No description</em>'}</p>
			<div class="row" style="margin-top: 12px;">
				<button data-act="add-gift" data-id="${w.id}" class="secondary">Add Gift</button>
				<button data-act="delete-wishlist" data-id="${w.id}" class="secondary" style="background: #fee2e2; color: #991b1b;">Delete</button>
			</div>`;
...
export function setupDashboard() {
	...
	createForm?.addEventListener('submit', async (e) => {
		...
		if (!title) {
			toast('Please enter a title for your wishlist', 'warn');
			return;
		}
		
		try { 
			await api('/wishlists', { method: 'POST', body: JSON.stringify({ title, description, is_public }) }); 
	...
	if (document.querySelector('#dashboard') && !document.querySelector('#dashboard')!.classList.contains('hidden')) { 
		void loadDashboard(); 
	}
}
...
async function openDetail(wishlistId: number) {
	try {
		...
		if (gifts.length === 0) {
			content.innerHTML = `<h3 style="margin-top:0">${w.title}</h3><p>${w.description||''}</p><p>No gifts yet. Add your first gift!</p>`+
		...
		content.addEventListener('click', async (e) => {
			const delBtn = (e.target as HTMLElement).closest('[data-act="del-gift"]');
			if (delBtn) {
				const giftId = delBtn.getAttribute('data-gid');
				if (confirm('Are you sure you want to delete this gift?')) {
					try {
						await api(`/gifts/${giftId}`, { method: 'DELETE' });
						toast('Gift deleted');
						await openDetail(wishlistId); // Refresh the view
					} catch (err: any) {
						toast(err.message, 'err');
					}
				}
...
function openModal(title: string, fields: {id:string,label:string,type:string,required?:boolean}[], onSubmit: (values: Record<string,string>)=>Promise<void>) {
	...
	form.onsubmit = async (e)=>{ 
		...
		fields.forEach(f=>{ 
			values[f.id] = (document.querySelector(`#modal-${f.id}`) as HTMLInputElement).value; 
		}); 
